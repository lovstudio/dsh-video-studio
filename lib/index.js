...
var StudioStore = class {
  ...
  async directory(name2) {
    const directory = join2(this.root, name2);
...
var MAX_ENTRIES = 200;
var skipped = (name2) => name2.startsWith(".") || HIDDEN_DIRECTORIES.has(name2);
      ...
      const detected = dependencies.some(
        (value) =>
          ...
          Object.keys(value).some(
            (name2) => name2 === "remotion" || name2.startsWith("@remotion/"),
          ),
      );
...
async function executionScope(exec, runtime) {
  exec.signal.throwIfAborted();
  const session = exec.agent?.session;
  if (!session?.header.cwd)
    throw new Error(
      "\u89C6\u9891\u5DE5\u5177\u9700\u8981\u4E00\u4E2A\u5DF2\u5173\u8054\u5DE5\u4F5C\u533A\u7684 DSH \u4F1A\u8BDD",
    );
  const workspacePath = await runtime.resolveWorkspace(session.id, exec.signal);
  exec.signal.throwIfAborted();
  return { sessionId: session.id, workspacePath };
}
async function belongsToWorkspace(project, scope) {
  if (!project.dsh) return false;
  try {
    return (await realpath3(project.dsh.workspacePath)) === scope.workspacePath;
  } catch (error) {
...
function jobValue(projectId, job) {
  return {
    projectId,
    jobId: job.id,
    status: job.status,
    progress: job.progress,
    ...(job.message !== void 0 ? { message: job.message } : {}),
    ...(job.outputUrl !== void 0 ? { outputUrl: job.outputUrl } : {}),
  };
}
function registerVideoStudioTools(ctx, runtime) {
  const observations = /* @__PURE__ */ new Map();
  const ownedJobs = /* @__PURE__ */ new Map();
  const disposers = [];
  const readProject = async (id2, scope) => {
    const project = await runtime.store.project(id2);
  ...
  const observe = (scope, project) => {
    let session = observations.get(scope.sessionId);
    if (!session)
      observations.set(scope.sessionId, (session = /* @__PURE__ */ new Map()));
  ...
  return async () => {
    ...
    observations.clear();
    await Promise.allSettled(
      [...ownedJobs.keys()].map((id2) => runtime.jobs.cancel(id2)),
    );
    ownedJobs.clear();
  };
}
...
async function apply(ctx, config = {}) {
  const lib = dirname2(fileURLToPath(import.meta.url));
