...
async function executionScope(exec) {
  ...
  const workspacePath = await realpath2(session.header.cwd);
  exec.signal.throwIfAborted();
...
async function belongsToWorkspace(project, scope) {
  ...
  try {
    return (await realpath2(project.dsh.workspacePath)) === scope.workspacePath;
  } catch (error) {
...
var inject = ["webServer", "connection", "tools"];
